...
describe Simp::Cli::Config::Item do
  ...
  describe '#determine_value_from_default' do
    it 'sets value when valid default can be determined' do
      ci = TestItem.new
      ci.key = 'test::key'
      expect(ci).to receive(:default_value_noninteractive).and_return('valid default')
      expect(ci).to receive(:validate).with('valid default').and_return(true)
  ...
  #  context 'when allow_queries=false and force_defaults=false' do
  #  context 'when allow_queries=false and force_defaults=true' do
  #  context 'when allow_queries=true  and force_defaults=false' do
  #  context 'when allow_queries=true  and force_defaults=true' do
  #
  describe '#determine_value_with_override' do
    ...
    context 'invalid preset value' do
      it 'fails if allow_queries and force_defaults are both false'
      it 'ignores silent during validate() twice when silent==true'
