...
class MyBadTestAddServerClassAction < Simp::Cli::Config::AddServerClassActionItem 
  def initialize(puppet_env_info = DEFAULT_PUPPET_ENV_INFO)
    # missing @class_to_add
    super(puppet_env_info)
    @key = 'puppet::add_my_bad_test_class_to_server'
  end
end
  ...
  describe '#apply' do
    ...
    context 'with a valid cli::network::hostname Item' do
      ...
      it "adds specified class to existing 'simp::server::classes' in <host>.yaml" do
        file = File.join(@files_dir, 'host_template_with_simp_server_classes.yaml')
        FileUtils.copy_file file, @host_file

        @ci.apply
        expect( @ci.applied_status ).to eq :succeeded
        expected = File.join(@files_dir, 'host_with_simp_server_classes.yaml')
      ...
      it "merges specified class into appropriate classes array in <host>.yaml" do
        file = File.join(@files_dir, 'host_with_simp_classes.yaml')
        FileUtils.copy_file file, @host_file
    ...
    context 'miscellaneous errors' do
      ...
      it 'fails with an exception when derived class does not set @class_to_add exist' do
        ci_bad = MyBadTestAddServerClassAction.new(@puppet_env_info)
        ci_bad.silent  = true
        ...
        expect( ci_bad.applied_status ).to eq :failed
      end
    end
  end
